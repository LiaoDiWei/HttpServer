      ...
      m_stop(false)
{
    ...
    for (int i = 0; i<process_number; ++i)
    {
        ...
        assert(ret == 0);
        m_sub_process[i].m_pid = fork();    //子进程执行fork之后的代码,子进程拥有父进程的一切,包括类实例对象
...
void processpool<T>::run_child()
{
    ...
    epoll_event events[MAX_EVENT_NUMBER];   //接收epoll_wait结果
                ...
                else
                {
                    for (int i = 0; i<ret; ++i)
                    {
                        switch (signals[i])
                        {
                            ...
                            case SIGINT:
                            {
                                /*如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部结束。当然，通知子进程结束更好的方法是向父、子进程之间的通信管道发送特殊数据，读者不
    妨自己实现之*/
